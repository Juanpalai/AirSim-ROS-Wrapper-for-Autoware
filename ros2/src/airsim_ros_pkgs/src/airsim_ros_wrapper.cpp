...
void AirsimROSWrapper::create_ros_pubs_from_settings_json()
{
    ...
    for (const auto& curr_vehicle_elem : AirSimSettings::singleton().vehicles) {
        ...
        if (airsim_mode_ == AIRSIM_MODE::DRONE) {
            ...
            car->current_velocity = nh_->create_publisher<geometry_msgs::msg::TwistStamped>("/vehicle/status/velocity_status", 10);
        }
...
sensor_msgs::msg::PointCloud2 AirsimROSWrapper::get_lidar_msg_from_airsim(const msr::airlib::LidarData& lidar_data, const std::string& vehicle_name, const std::string& sensor_name) const
{
    ...
    lidar_msg.header.frame_id = "os_sensor";
    ...
    if (lidar_data.point_cloud.size() > 3) {
        ...
        if (isENU_) {
            try {
                ...
                auto transformStampedENU = tf_buffer_->lookupTransform(AIRSIM_FRAME_ID, vehicle_name, rclcpp::Time(0), rclcpp::Duration(duration.count()));
...
void AirsimROSWrapper::publish_vehicle_state()
{
    for (auto& vehicle_name_ptr_pair : vehicle_name_ptr_map_) {
        ...
        if (airsim_mode_ == AIRSIM_MODE::CAR) {
            ...
            auto car = static_cast<CarROS*>(vehicle_ros.get());
            geometry_msgs::msg::TwistStamped tagnova_speed;
            tagnova_speed.twist.linear.x = car->car_state_msg_.speed;
            car->current_velocity->publish(tagnova_speed);
