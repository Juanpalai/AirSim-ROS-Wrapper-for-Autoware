...
void AirsimROSWrapper::create_ros_pubs_from_settings_json()
{
    ...
    for (const auto& curr_vehicle_elem : AirSimSettings::singleton().vehicles) {
        ...
        if (airsim_mode_ == AIRSIM_MODE::DRONE) {
            ...
            car->current_velocity = nh_private_.advertise<geometry_msgs::TwistStamped>("current_velocity", 10);
        }
...
sensor_msgs::PointCloud2 AirsimROSWrapper::get_lidar_msg_from_airsim(const msr::airlib::LidarData& lidar_data, const std::string& vehicle_name, const std::string& sensor_name) const
{
    ...
    lidar_msg.header.frame_id = "airsim_sensor"; //FilterFrameID( sensor_name)
...
void AirsimROSWrapper::publish_vehicle_state()
{
    for (auto& vehicle_name_ptr_pair : vehicle_name_ptr_map_) {
        ...
        if (airsim_mode_ == AIRSIM_MODE::CAR) {
            ...
            auto car = static_cast<CarROS*>(vehicle_ros.get());
            geometry_msgs::TwistStamped tagnova_speed;
            tagnova_speed.twist.linear.x = car->car_state_msg.speed;
